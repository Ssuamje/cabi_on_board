...
class BotTests extends OutputTest {
	...
	private final static String 코카콜라 = "<코카콜라>";
	private final static String 데자와 = "<데자와>";
	private final static String 이랑 = "이랑";
	private final static String 랑 = "랑";
	...
	void test() {
		...
		orderTester.when(orderBiProduct(파타타스프리타스, 랑, 코카콜라)).then(yes(12000));
		orderTester.when(removeProduct(데자와)).then(tellNotOrdered(데자와));
		orderTester.when(removeProduct(파타타스프리타스)).then(yes());
		orderTester.when(askHowMuch()).then(tellResult(13000));
	}
